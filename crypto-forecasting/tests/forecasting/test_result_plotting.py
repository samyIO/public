...
class TestResultPlotter(unittest.TestCase):
    ...
    def create_mock_ohlcv_data(self, timeframe):
        ...
        price_data = [
            base_price * (1 + 0.001 * i + 0.0005 * np.random.randn())
    ...
    def create_y_df(self, ohlcv_df):
        ...
        y_df = pd.DataFrame({"ds": returns.index, "y": returns.values})
    ...
    def create_forecast_df(self, ohlcv_df):
        ...
        forecast_df = pd.DataFrame({"ds": future_dates, "NHITS": forecast_returns})
    ...
    def test_plot_absolute_prices_properties(self, mock_savefig):
        ...
        fig = self.plotter.plot_absolute_prices(
            figsize=(10, 5), timeframe_configs=self.timeframe_configs, timeframe="1h"
        )
    ...
    def test_plot_absolute_prices_different_timeframes(self, mock_savefig):
        ...
        for timeframe in timeframes:
            ...
            # Execute the plotting method
            fig = self.plotter.plot_absolute_prices(
                timeframe_configs=self.timeframe_configs, timeframe=timeframe
            )
    ...
    def test_error_handling(self):
        ...
        temp_plotter = ResultPlotter(
            symbol=self.symbol, ohlcv_df=self.ohlcv_df_1h, y_df=None, forecast_df=None
        )
