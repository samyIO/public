...
class TestFeatureCreator(unittest.TestCase):
    ...
    def test_create_nhits_features(self, mock_logging):
        ...
        expected_features = self.feature_creator.get_all_feature_names() + [
            "target_next_return"
        ]
    ...
    def test_create_nhits_features_data_quality(self):
        ...
        self.assertTrue(
            (feature_df["rsi_14"] >= 0).all() and (feature_df["rsi_14"] <= 100).all()
        )
        ...
        self.assertTrue(
            np.issubdtype(feature_df["target_next_return"].dtype, np.number)
        )
