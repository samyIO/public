...
class TestDataValidator(unittest.TestCase):
    ...
    def create_sample_ohlcv_data(self):
        ...
        for i in range(100):
            ...
            high_price = max(open_price, close_price) * (1 + abs(random_state.normal(0, 0.003)))
            low_price = min(open_price, close_price) * (1 - abs(random_state.normal(0, 0.003)))
    ...
    def create_problematic_ohlcv_data(self):
        ...
        df_reset.iloc[-1, df_reset.columns.get_indexer(['timestamp'])] = duplicate_timestamp
        df = df_reset.set_index('timestamp')
        
        # Introduce OHLC violations
        df.loc[df.index[30], 'high'] = df.loc[df.index[30], 'low'] * 0.9  # High less than low
        df.loc[df.index[40], 'low'] = df.loc[df.index[40], 'open'] * 1.1  # Low greater than open
    ...
    def test_clean_data_default(self):
        ...
        self.assertEqual(cleaned_df.isnull().sum().sum(), 0, "Should have no NaN values")
        self.assertEqual(sum(cleaned_df.index.duplicated()), 0, "Should have no duplicates")
        
        # Check that OHLC relationships are fixed
        self.assertTrue((cleaned_df['high'] >= cleaned_df['low']).all(), "High should be >= Low")
        self.assertTrue((cleaned_df['high'] >= cleaned_df['open']).all(), "High should be >= Open")
        self.assertTrue((cleaned_df['high'] >= cleaned_df['close']).all(), "High should be >= Close")
        self.assertTrue((cleaned_df['low'] <= cleaned_df['open']).all(), "Low should be <= Open")
        self.assertTrue((cleaned_df['low'] <= cleaned_df['close']).all(), "Low should be <= Close")
        
        # Check rounding
        for col in self.validator.price_columns:
            decimal_count = cleaned_df[col].astype(str).str.split('.').str[1].str.len().max()
            self.assertLessEqual(decimal_count, self.validator.price_decimals, 
                                f"{col} should be rounded to {self.validator.price_decimals} decimals")
        
        for col in self.validator.volume_columns:
            decimal_count = cleaned_df[col].astype(str).str.split('.').str[1].str.len().max()
            self.assertLessEqual(decimal_count, self.validator.volume_decimals, 
    ...
    def test_clean_data_with_different_fill_methods(self):
        ...
        df_with_nans.iloc[10:15, df_with_nans.columns.get_indexer(['open', 'close'])] = np.nan
    ...
    def test_handle_price_anomalies(self):
        ...
        df_with_outlier.loc[df_with_outlier.index[outlier_idx], 'close'] = original_value * 20
    ...
    def test_validate_ohlc(self):
        ...
        df_with_violations.iloc[30, df_with_violations.columns.get_indexer(['high'])] = 9000  # Lower than expected
        df_with_violations.iloc[30, df_with_violations.columns.get_indexer(['low'])] = 11000  # Higher than expected
    ...
    def test_round_numbers(self):
        ...
        for col in self.validator.price_columns:
            decimal_places = rounded_df[col].astype(str).str.split('.').str[1].str.len().max()
    ...
    def test_check_ohlc_violations(self):
        ...
        df_with_violations = self.regular_df.copy()
        df_with_violations.loc[df_with_violations.index[10], 'high'] = 9000  # Set high below other values
