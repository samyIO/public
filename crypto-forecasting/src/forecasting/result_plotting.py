...
class ResultPlotter:
    ...
    def convert_returns_to_prices(self):
        ...
        if self.y_df is None or self.forecast_df is None:
            raise ValueError("Both historical data and forecast data "
        ...
        historical_prices = pd.DataFrame({
            'ds': self.ohlcv_df['timestamp'],
            'price': self.ohlcv_df['close']
        })
        
        # Merge with the y_df to get matching timestamps
        y_df_with_prices = pd.merge(
            y_df_copy,
            historical_prices,
            on='ds',
            how='left'
    ...
    def plot_absolute_prices(self, figsize=(14, 7), title=None, 
                             timeframe_configs=None, timeframe="1h"):
        """
        ...
        ax.plot(recent_history['ds'], recent_history['price'], 
                label='Historical', color='blue', linewidth=2)
        
        # Plot forecasted prices
        ax.plot(forecast_df['ds'], forecast_df['absolute_price'], 
