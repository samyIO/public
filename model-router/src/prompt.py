class PromptCentral:
    ...
    def get_router_prompt():
        ...
        to the user query, that indicates the level of complexity of it. 
        Generate a score between 0.1 (easy to answer) and 1.0 (require thinking). 
        
        Generate your response in valid JSON format. 
        Only return a realistic single float value, the score. 
    ...
    def embed_in_tags(user_input, tag_type):
        ...
        Args:
            ...
            tag_type (str): The type of tag to use
            (e.g., 'begin_of_text', 'header_id', 'system')
        ...
        if tag_type not in tag_mappings:
            raise ValueError(
                f"Unknown tag type: {tag_type}. Available tag types: "
                f"{list(tag_mappings.keys())}"
            )
