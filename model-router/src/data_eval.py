...
class AdaptiveRouter:
    ...
    def run_data_creation(self, metric):
        ...
        for q in questions:
            ...
            llm = init_chat_model(
                "llama3.2:3b-instruct-q8_0", model_provider="ollama", temperature=0.6
            )
            scores = []
            ...
            for run in range(5):
                ...
                while not score_generated and attempts < 2:
                    try:
                        ...
                        print(
                            f"Successfully generated score {run + 1}/5 for question: {q[:50]}..."
                        )
            ...
            test_set[q] = {
                ...
                "variance": (
                    sum(
                        ...
                        ** 2
                        for x in [s for s in scores if s is not None]
                    )
                    / len([s for s in scores if s is not None])
                    if scores
                    else None
    ...
    def calculate_threshold(self, result_data):
        ...
        try:
            ...
            simple_questions = [
                q for q in result_data["test_results"] if q["complexity"] == "easy"
            ]
            complex_questions = [
                q for q in result_data["test_results"] if q["complexity"] == "hard"
            ]
