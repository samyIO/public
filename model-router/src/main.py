    ...
    while True:
        ...
        while i < 2 and parsed_object == "":
            ...
            try:
                ...
                router_prompt = tagged_prompt + PromptCentral.embed_in_tags(user_query, "user")
        ...
        # A major challenge is the conversation history management due the mechanics of model distillation
        # If routing should be used in extensive relational tasks, using the same conversation history for
        # both model routes would lead to a reverse distillation, for the run time period.
        # The responses of the smaller model impact the performance cap of the bigger model due to context attention.
        # For categorization applications this doesn't matter since the models would amost never share the same context
